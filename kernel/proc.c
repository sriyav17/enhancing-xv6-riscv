...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // copies mask from parent to child
  np->mask = p->mask;
...
void fcfs(struct cpu *c)
{
  ...
  for (struct proc *p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state == RUNNABLE)
    {
      if (!minproc || p->creation_time < minproc->creation_time)
      {
        if (minproc)
...
int set_SP(int priority, int pid)
{
  for (struct proc *p = proc; p < &proc[NPROC]; p++)
  {
    if (myproc() == p)
...
get_DP(struct proc *p)
{
  ...
  p->niceness = niceness;
  uint64 DP = max(0, min(p->static_priority - niceness + 5, 100));
  return DP;
}
  ...
  for (struct proc *p = proc; p < &proc[NPROC]; p++)
  {
    ...
    else
    {
      ...
      if (DP == min_priority)
      {
        ...
        if (minproc->no_of_times_scheduled == p->no_of_times_scheduled)
        {
        ...
        {
          if (p->no_of_times_scheduled < minproc->no_of_times_scheduled)
          {
            release(&minproc->lock);
            minproc = p;
          }
...
void procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printf("%d\t%d\t\t%s\t%d\t%d\t%d\n", p->pid, get_DP(p), state, p->run_time, ticks - p->creation_time - p->run_time, p->no_of_times_scheduled);
    printf("\n");
  }
#endif
